...
function heuristicQuestions(taskType, agent, state) {
    const all = {
        youtube: [
            {
                id: uid(),
...
function buildHeuristicPlan(task, agent, answers) {
    ...
    const message = taskType === "youtube"
        ? readyForExecution
...
async function buildGeminiPlan(task, agent, answers) {
    if (!gemini)
        return null;
    const prompt = `
...
async function getPlan(task, agent, answers) {
...
function fallbackReason(type) {
    ...
    return {
        ...
        conf: r(84, 93),
        finalDecision: "Proceed with conditional execution. The strategic direction is valid, but the decision should be gated by validation checkpoints, measurable success criteria, and explicit rollback conditions. Use a phased approach, empower a risk owner to pause the plan if assumptions break, and move to full commitment only after early signals confirm viability.",
        tradeoffs: [
            "Speed vs validation",
            "Momentum vs caution",
...
async function buildReasonChain(query) {
    if (!gemini) {
        return {
...
${query}
`;
    const exec = (await generateJson(execPrompt)) || fallbackReason("exec");
    const skepPrompt = `
...
${JSON.stringify(exec)}
`;
    const skep = (await generateJson(skepPrompt)) || fallbackReason("skep");
    const synthPrompt = `
...
${JSON.stringify(skep)}
`;
    const synth = (await generateJson(synthPrompt)) || fallbackReason("synth");
...
app.post("/chat", async (req, res) => {
    ...
    if (!gemini) {
        return res.json({
            message: "NODUS is online. For general questions, connect the Gemini key in your api_key file. For decisions and browser tasks, the system can still run with local fallbacks."
        });
    }
    try {
        const response = await gemini.models.generateContent({
...
app.post("/task", async (req, res) => {
    const { task, agent, answers } = req.body;
    if (!task || !task.trim()) {
        return res.status(400).json({ error: "Task is required" });
    }
    const safeAgent = agent === "Septic" || agent === "Commander" || agent === "Executive"
        ? agent
        : "Commander";
    const safeAnswers = answers && typeof answers === "object" && !Array.isArray(answers)
        ? Object.fromEntries(Object.entries(answers).filter(([, value]) => typeof value === "string" && value.trim().length > 0))
        : {};
    try {
...
app.post("/execute", async (req, res) => {
    const { taskType, knownState, submittedTask } = req.body;
    try {
